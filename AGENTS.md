...
- 每次写完代码必须执行与改动范围匹配的检查；前端至少运行类型检查，涉及页面行为或发版时还要执行生产构建和真实页面回归，不能把发现回归的责任留给用户。
- 不要改无关文件，不要顺手重构。
...
- 页面文案保持中文。
- 所有用户可见中文文案必须同时提供英文显示；静态文案进入统一词典，带数量、状态或用户数据的动态文案必须使用显式 formatter，不能依赖整段 DOM 文本匹配。
- 输入框的 `placeholder`、按钮和图标的 `aria-label` / `title`、Toast、确认框、错误卡片与空状态都属于国际化范围。
- 用户输入、历史提示词、素材正文、画布名、节点正文和 Agent 消息必须保留原语言，并用 `data-no-i18n="true"` 或等效的组件级隔离避免全局翻译器改写。
...
- 发版本前必须运行 `cd web && bun run check && bun run build`；涉及 UI 或国际化时，还必须在中英文模式下逐页检查可见文本及 `placeholder`、`aria-label`、`title`，确认动态状态和危险操作弹窗无混合语言后才能发布。
