...
import { isKling3TurboVideoModel, isMiniMaxHailuoVideoModel, isSeedanceVideoModel, mediaModelCapability, normalizeAspectRatio, normalizeKling3TurboResolution, normalizeSeedanceResolution, normalizeVideoDurationForModel, normalizeVideoOperation, seedanceAspectRatioOptions, seedanceForcesAdaptiveRatio, seedanceResolutionOptions, supportsVideoAudioGeneration, supportsVideoResolution, videoDurationOptions } from "@/lib/anyaigc-media-models";
...
export function VideoSettingsPanel({ config, onConfigChange, theme, showTitle = true, className = "w-[320px] space-y-4 rounded-2xl px-1 py-0.5" }: VideoSettingsPanelProps) {
    ...
    const ratio = normalizeAspectRatio(config.size, model);
    ...
    const selectableOperations = hailuo || seedance;
    const hidesAspectRatio = seedanceForcesAdaptiveRatio(model, operation);
    const ratios = seedance ? seedanceAspectRatioOptions() : ["16:9", "9:16", "1:1"];
    const resolution = seedance ? normalizeSeedanceResolution(model, config.vquality) : normalizeKling3TurboResolution(config.vquality);
    const resolutions = seedance ? seedanceResolutionOptions(model) : ["720p", "1080p"];
    ...
    return <ImageSettingsTheme theme={theme}><div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
        ...
        {capability?.kind === "video" ? <SettingGroup title={canvasText("生成方式", "Generation mode", language)} color={theme.node.muted}>{selectableOperations ? <div className="grid grid-cols-3 gap-2">{capability.operations.map((value) => <Option key={value} selected={operation === value} theme={theme} onClick={() => onConfigChange("videoOperation", value)}>{operationLabel(value, language)}</Option>)}</div> : <div className="rounded-xl border px-3 py-2.5 text-sm" style={{ borderColor: mutedBorderColor(theme) }}>{operationLabel(operation, language)}</div>}</SettingGroup> : null}
        {motionControl ? <div className="rounded-xl border px-3 py-2.5 text-sm leading-6" style={{ borderColor: mutedBorderColor(theme), color: theme.node.muted }}>{canvasText("需要 1 张人物参考图和 1 个动作参考视频。", "Requires one person image and one motion-reference video.", language)}</div> : <>
            {supportsVideoResolution(model) ? <SettingGroup title={canvasText("分辨率", "Resolution", language)} color={theme.node.muted}><div className={`grid gap-2 ${resolutions.length > 3 ? "grid-cols-4" : resolutions.length > 2 ? "grid-cols-3" : "grid-cols-2"}`}>{resolutions.map((value) => <Option key={value} selected={resolution === value} theme={theme} onClick={() => onConfigChange("vquality", value)}>{value}</Option>)}</div></SettingGroup> : null}
            {hidesAspectRatio ? null : <SettingGroup title={canvasText("画面比例", "Aspect ratio", language)} color={theme.node.muted}><div className={`grid gap-2 ${ratios.length > 3 ? "grid-cols-4" : "grid-cols-3"}`}>{ratios.map((value) => <Option key={value} selected={ratio === value} theme={theme} onClick={() => onConfigChange("size", value)}>{value}</Option>)}</div></SettingGroup>}
            ...
            {supportsVideoAudioGeneration(model) ? <SettingGroup title={canvasText("生成音频", "Generate audio", language)} color={theme.node.muted}><div className="grid grid-cols-2 gap-2">{[{ value: "true", zh: "开", en: "On" }, { value: "false", zh: "关", en: "Off" }].map((item) => <Option key={item.value} selected={(config.videoGenerateAudio || "true") === item.value} theme={theme} onClick={() => onConfigChange("videoGenerateAudio", item.value)}>{language === "en" ? item.en : item.zh}</Option>)}</div></SettingGroup> : null}
        </>}
...
export function videoResolutionLabel(value: string, model: string, language = useLanguageStore.getState().language) { return isSeedanceVideoModel(model) ? normalizeSeedanceResolution(model, value) : isKling3TurboVideoModel(model) ? normalizeKling3TurboResolution(value) : canvasText("模型默认", "Model default", language); }
...
export function videoSizeLabel(value: string, model = "", _language = useLanguageStore.getState().language) { return normalizeAspectRatio(value, model); }
